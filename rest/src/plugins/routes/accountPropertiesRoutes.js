...
module.exports = {
	register: (server, db, { config }) => {
		const accountIdToAddress = (type, accountId) => ((AccountType.publicKey === type)
			? address.publicKeyToAddress(accountId, networkInfo.networks[config.network.name].id)
			: accountId);

		const accountPropertiesSender = routeUtils.createSender('accountProperties.accountProperties');

		server.get('/accountProperties/:accountId', (req, res, next) => {
		...
		server.post('/accountProperties', (req, res, next) => {
