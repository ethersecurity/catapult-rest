...
describe('account properties routes', () => {
	...
	describe('get by address', () => {
		test.route.document.addGetPostDocumentRouteTests(accountPropertiesRoutes.register, {
			routes: { singular: '/accountProperties/:accountId', plural: '/accountProperties' },
			inputs: {
				valid: {
		...
		test.route.document.addGetPostDocumentRouteTests(accountPropertiesRoutes.register, {
			...
			inputs: {
				valid: {
					...
					parsed: [address.publicKeyToAddress(convert.hexToUint8(publicKeys.valid[0]), networkInfo.networks.mijinTest.id)],
